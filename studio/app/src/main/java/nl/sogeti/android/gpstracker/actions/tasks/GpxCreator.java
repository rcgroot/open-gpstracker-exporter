 ...
 * @version $Id$
 */
public abstract class GpxCreator extends XmlCreator {
    ...
    protected GpxCreator(Context context, Uri trackUri, ProgressListener listener) {
        super(context, trackUri, listener);
    }
     ...
     * @return
     */
    protected void exportGpx(OutputStream outputStream) {
        BufferedOutputStream buf = null;
        try {
            buf = new BufferedOutputStream(outputStream, 8 * 8192);
            serializeTrack(mTrackUri, buf);
        } catch (IllegalStateException e) {
            String text = mContext.getString(R.string.ticker_failed) + " \"" + mTrackUri + "\" " + mContext.getString
                    (R.string.error_buildxml);
            handleError(mContext.getString(R.string.taskerror_gpx_write), e, text);
        } catch (IOException e) {
            String text = mContext.getString(R.string.ticker_failed) + " \"" + mTrackUri + "\" " + mContext.getString(R.string.error_writefailed);
            handleError(mContext.getString(R.string.taskerror_gpx_write), e, text);
        } finally {
            if (buf != null) {
                try {
